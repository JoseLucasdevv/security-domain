...
import java.util.regex.Pattern;
    ...
    public class UserUpdateGeneralValidation {
        ...
        public ResponseUserUpdateValidation validationUpdate(User userAlreadyExist, UserUpdateDTO userUpdateValidation){
            ...
            this.validatePassword(userAlreadyExist,userUpdateValidation.oldPassword(),userUpdateValidation.newPassword());

            if(!this.validateUpdate) new Exception(new ResponseUserUpdateValidation(userAlreadyExist,this.resultValidation).toString(),HttpStatus.BAD_REQUEST);
        ...
        private void validateUsername(User userAlreadyExist,String username) {
            if(username != null && !username.isBlank() && username.length() >= 3 && username.length() <= 55){
                ...
                if(this.userRepository.findByUsername(username.trim()) != null) {
        ...
        private void validatePassword(User userAlreadyExist, String oldPassword,String newPassword){

            if(newPassword != null && !newPassword.isBlank() && oldPassword != null && !oldPassword.isBlank()){
                boolean currPasswordValidation = this.passwordEncoder.matches(oldPassword,userAlreadyExist.getPassword());
                System.out.println("value of matches from oldPassword" + currPasswordValidation);
                boolean newPasswordCompare = this.passwordEncoder.matches(newPassword,userAlreadyExist.getPassword());
                System.out.println("value of matches from newPassword" + currPasswordValidation);
                System.out.println(newPasswordCompare);
                if(!currPasswordValidation) {
                    System.out.println("field old password here");
                   resultValidation.put("fieldOldPassword","the Old password doesn't matches");
                   return;
                }
                if(newPasswordCompare){
                    System.out.println("field new password here");
                    resultValidation.put("fieldNewPassword","the current password can't be the same to the current");
                    return;
                }
